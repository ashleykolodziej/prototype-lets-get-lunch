...
async function buildSlidersByLocation(data, $f7) {
		   ...
		   const swiperWrapper = document.createElement('div');
			...
			for ( const dish of item.items ) {
				...
				if (
					...
					// Something about this isn't working right now.
					// It's only returning one all veg menu.
					if (!isVeg) {
						return;
					}
					...
					swiper.appendSlide(`<div
							...
							<div class="card-footer">in ${dish.item.categoryName}</p>
							</div>
						</div>`);
					}
  			}
...
export default buildSlidersByLocation;
